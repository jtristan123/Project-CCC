...
min_conf_threshold = float(.1)#(args.threshold)
...
while T:
    ...
    while not centered: #added this <-------------------------------------------------------
        ...
        input_dtype = input_details[0]['dtype'] #NEW
        # Normalize pixel values if using a floating model (i.e. if model is non-quantized)
        if floating_model:
            input_data = (np.float32(input_data) - input_mean) / input_std
        else:
            input_data = input_data.astype(input_dtype) #NEW
        ...
        print("Getting tensor outputs...")

        boxes = interpreter.get_tensor(output_details[boxes_idx]['index'])[0] # Bounding box coordinates of detected objects
        classes = interpreter.get_tensor(output_details[classes_idx]['index'])[0] # Class index of detected objects
        scores = interpreter.get_tensor(output_details[scores_idx]['index'])[0] # Confidence of detected objects
        print("Done getting tensor outputs.")
        ...
        #for i in range(len(scores)):#find all the matching objects more than one
            ...
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                print("Score passed threshold")

                # Get bounding box coordinates and draw box
                # Interpreter can return coordinates that are outside of image dimensions, 
                # need to force them to be within image using max() and min()
                print("Raw box:", boxes[i])

                ymin = int(max(1,(boxes[i][0] * imH)))
                xmin = int(max(1,(boxes[i][1] * imW)))
                ymax = int(min(imH,(boxes[i][2] * imH)))
                xmax = int(min(imW,(boxes[i][3] * imW)))
                #shrink_ratio = 0.10  # shrink by 10%

                #width = xmax - xmin
                #height = ymax - ymin

                #xmin += int(width * shrink_ratio / 2)
                #xmax -= int(width * shrink_ratio / 2)
                #ymin += int(height * shrink_ratio / 2)
                #ymax -= int(height * shrink_ratio / 2)
                
                #width = int((min(imW,(boxes[i][3] * imW))) - (max(1,(boxes[i][1] * imW))))
                
                print(f"[DEBUG] box: ({xmin}, {ymin}), ({xmax}, {ymax})")
                ...
                if object_name == "cone": #only do this if cone is found
                    ...
                    print(f"[DEBUG] label_text: {label_text}")
