    ...
    while not centered: #added this <-------------------------------------------------------
        ...
        #for i in range(len(scores)):#find all the matching objects more than one
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                ...
                ymin = int(max(1,(boxes[i][0] * imH))) #top side of square
                xmin = int(max(1,(boxes[i][1] * imW)))
                ymax = int(min(imH,(boxes[i][2] * imH))) #bottom side of square
                ...
                if object_name == "cone": #only do this if cone is found
                    ...
                    if height > max_height:
                        max_height = height
                        closest_cone_index = i
                        print(f"Closest cone found at index {closest_cone_index} with height {max_height}")
        
        if cone_detected and closest_cone_index != -1:
            # Draw the bounding box around the cone
            i = closest_cone_index
            ymin = int(max(1,(boxes[i][0] * imH))) #top side of square
            xmin = int(max(1,(boxes[i][1] * imW)))
            ymax = int(min(imH,(boxes[i][2] * imH))) #bottom side of square
            xmax = int(min(imW,(boxes[i][3] * imW)))
            ...
            height = ymax - ymin
            bottom_offset = imH - ymax
            object_name = labels[int(classes[i])]

            # Draw bounding box and label
            label = f'{object_name}: {int(scores[i]*100)}% height={height} idx={i}'
            ...
            # Debug output
            print(f"[DEBUG] Tracking cone idx={i} at (x={center_x}, y={center_y}), height={height}, bottom_offset={bottom_offset}")
